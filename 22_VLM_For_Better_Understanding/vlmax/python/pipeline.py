...
# elif file_type not in ['jpg', 'jpeg', 'png']:
#     print("Unsupported file type. Please provide a PDF, JPG, JPEG, or PNG file.")
#     sys.exit(1)
...
def save(html_code):
    ...
    print(base_name)
    results["file_path"].append(os.path.join(dir, f'{base_name}.csv'))
    results["file_path"].append(os.path.join(dir, f'{base_name}.html'))
...
def detect_and_crop_tables(image, base_name, page_num, save_dir):
...
pics = {"pics": []}
def load_images():
    for file in os.walk("./preprocess_results/"):
        for name in file[2]:
            if not (name.endswith('structure.png') or name.endswith('boxes.png')):
                results["img_path"].append(os.path.join('./preprocess_results/', name))
                image_path = os.path.join(file[0], name)
                image = Image.open(image_path).convert("RGB")
                html_code = inference(image)
                save(html_code)
            else:
                pics["pics"].append(os.path.join('./preprocess_results/', name))
...
if __name__ == "__main__":
    ...
    results_dir = Path.cwd() / "preprocess_results"
